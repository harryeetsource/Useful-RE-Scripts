...
$url2 = "https://github.com/x64dbg/x64dbg/releases/download/snapshot/snapshot_2022-05-18_12-39.zip"
...
$url14 = "https://github.com/hasherezade/hollows_hunter/releases/download/v0.3.4/hollows_hunter64.exe"
$folder14 = "$env:temp\hollows"
...
if (Test-Path -Path $folder14) { Write-Host "cyberchef directory already exists, skipping" }
    ...
    New-Item -Path "$env:temp\" -Name "hollows" -ItemType "directory"
    Invoke-WebRequest  $url14 -OutFile "$env:Temp\hollows\hollowshunter.exe"
    }
...
if (Test-Path -Path $folder22) {Write-Host "sysinternals directory already exists"}
    ...
    Expand-Archive -LiteralPath "$env:temp\sysinternals\suite.zip" -DestinationPath "$env:temp\sysinternals\suite\"
